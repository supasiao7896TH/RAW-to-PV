...
function saveTags() { localStorage.setItem(STORAGE_KEY, JSON.stringify(tags)); }
function nextId() { return Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }

function calcPV(rawPct, svh, svl) { return svl + (rawPct / 100) * (svh - svl); }
...
// ± SIGN TOGGLE — Samsung keyboard has no minus key
...
function setupTagSearch(inputId, dropdownId, onSelect) {
  ...
  function render(query) {
    ...
    const filtered = tags.filter(t => t.name.toLowerCase().includes(q) || (t.description || '').toLowerCase().includes(q));
    dropdown.innerHTML = '';
    if (filtered.length === 0) {
      ...
      filtered.slice(0, 20).forEach(t => {
        ...
        item.innerHTML = `<span class="dropdown-tag-name">${escHtml(t.name)}</span><span class="dropdown-tag-info">SVH: ${t.svh} / SVL: ${t.svl}<br><small>${escHtml(t.unit || '')} ${t.description ? '· ' + escHtml(t.description) : ''}</small></span>`;
        item.addEventListener('mousedown', e => { e.preventDefault(); onSelect(t); input.value = t.name; dropdown.classList.remove('open'); });
        dropdown.appendChild(item);
      });
    }
    dropdown.classList.add('open');
  }
  input.addEventListener('focus', () => { if (tags.length > 0) render(input.value); });
  input.addEventListener('input', () => render(input.value));
  document.addEventListener('click', e => { if (!input.closest('.tag-search-wrapper').contains(e.target)) dropdown.classList.remove('open'); });
}
...
document.getElementById('calc-raw').addEventListener('keydown', e => { if (e.key === 'Enter') runCalc(); });
...
function runCalc() {
  ...
  if (isNaN(rawPct) || isNaN(svh)) { toast('กรุณากรอกค่า RAW% และ SVH', 'error'); return; }
  if (isNaN(svl)) { toast('ค่า SVL ไม่ถูกต้อง', 'error'); return; }
  if (rawPct < 0 || rawPct > 100) { toast('ค่า RAW% ต้องอยู่ระหว่าง 0 – 100', 'error'); return; }
  const pv = calcPV(rawPct, svh, svl);
  resultEmpty.style.display = 'none';
  resultDisplay.style.display = 'block';
  document.getElementById('result-tag-name').textContent = tagName || 'Custom Calculation';
  document.getElementById('result-pv-number').textContent = formatNum(pv);
  document.getElementById('result-pv-unit').textContent   = unit;
  document.getElementById('result-breakdown').textContent = `RAW = ${rawPct}%  |  SVH = ${svh}  |  SVL = ${svl}  |  Span = ${svh - svl}`;
  document.getElementById('result-formula').textContent   = `PV = ${svl} + (${rawPct}/100) × (${svh}−${svl}) = ${formatNum(pv)}`;
...
function renderQuickRef(svh, svl, unit) {
  ...
  rows.innerHTML = pcts.map(p => `<div class="qr-row"><span class="qr-raw">${p}%</span><span class="qr-pv">${formatNum(calcPV(p, svh, svl))} ${unit || ''}</span></div>`).join('');
  document.getElementById('quick-ref').style.display = 'block';
}
...
function renderLibrary(query = '') {
  ...
  filtered.forEach(tag => {
    ...
    tr.innerHTML = `<td class="td-tag-name">${escHtml(tag.name)}</td><td class="td-desc">${escHtml(tag.description || '—')}</td><td class="td-number text-right">${tag.svh}</td><td class="td-number text-right">${tag.svl}</td><td class="td-unit">${escHtml(tag.unit || '—')}</td><td><div class="td-actions"><button class="btn-icon use" data-id="${tag.id}">▶ ใช้</button><button class="btn-icon edit" data-id="${tag.id}">✏</button><button class="btn-icon delete" data-id="${tag.id}">✕</button></div></td>`;
...
document.getElementById('library-tbody').addEventListener('click', e => {
  ...
  if (btn.classList.contains('delete')) {
    if (confirm(`ลบ Tag "${tag.name}" ?`)) { tags = tags.filter(t => t.id !== id); saveTags(); renderLibrary(document.getElementById('lib-search').value); toast(`ลบ ${tag.name} แล้ว`, 'success'); }
  } else if (btn.classList.contains('edit')) { openModal(tag); }
  else if (btn.classList.contains('use')) { switchToCalcWith(tag); }
});
...
function saveModal() {
  ...
  if (isNaN(svl))                  { showModalError('ค่า SVL ไม่ถูกต้อง'); return; }
  if (svh <= svl)                  { showModalError('SVH ต้องมากกว่า SVL'); return; }
  if (tags.find(t => t.name === name && t.id !== editingId)) { showModalError(`Tag "${name}" มีในระบบแล้ว`); return; }
  errEl.style.display = 'none';
  if (editingId) { const idx = tags.findIndex(t => t.id === editingId); tags[idx] = { ...tags[idx], name, unit, description: desc, svh, svl }; }
  else tags.push({ id: nextId(), name, unit, description: desc, svh, svl });
  saveTags(); renderLibrary(document.getElementById('lib-search').value); closeModal();
  toast(editingId ? `อัปเดต ${name} แล้ว` : `เพิ่ม ${name} แล้ว`, 'success');
}

function showModalError(msg) { const el = document.getElementById('modal-error'); el.textContent = msg; el.style.display = ''; }

// EXPORT / IMPORT CSV
document.getElementById('export-csv-btn').addEventListener('click', exportCSV);
document.getElementById('import-csv-btn').addEventListener('click', () => document.getElementById('import-file-input').click());
...
document.getElementById('clear-all-btn').addEventListener('click', () => {
  ...
  if (confirm(`ลบ Tag ทั้งหมด ${tags.length} รายการ?`)) { tags = []; saveTags(); renderLibrary(); toast('ล้างข้อมูลทั้งหมดแล้ว', 'success'); }
});

function exportCSV() {
  if (tags.length === 0) { toast('ไม่มีข้อมูลให้ export', 'error'); return; }
  const rows = tags.map(t => [t.name, t.description || '', t.svh, t.svl, t.unit || ''].map(v => `"${String(v).replace(/"/g, '""')}"`).join(','));
  downloadFile(['Tag Name,Description,SVH,SVL,Unit', ...rows].join('\n'), 'dcs_tag_library.csv', 'text/csv');
...
function importCSV(e) {
  ...
  reader.onload = ev => {
    ...
    lines.slice(1).forEach(line => {
      ...
      const name = cols[0].trim().toUpperCase(); const svh = parseFloat(cols[2]); const svl = parseFloat(cols[3]) || 0;
      if (!name || isNaN(svh) || tags.find(t => t.name === name)) { skipped++; return; }
      tags.push({ id: nextId(), name, description: cols[1] || '', svh, svl, unit: cols[4] || '' }); added++;
    });
    saveTags(); renderLibrary(); toast(`Import สำเร็จ: เพิ่ม ${added} tag, ข้าม ${skipped}`, 'success');
  };
...
function runBatch() {
  ...
  if (svhVal === '' || isNaN(svh)) { toast('กรุณากรอกค่า SVH', 'error'); return; }
  if (isNaN(svl))  { toast('ค่า SVL ไม่ถูกต้อง', 'error'); return; }
  if (svh <= svl)  { toast('SVH ต้องมากกว่า SVL', 'error'); return; }
  const rawValues = rawText.split(/[\n,]+/).map(v => v.trim()).filter(v => v !== '').map(v => parseFloat(v)).filter(v => !isNaN(v));
  if (rawValues.length === 0) { toast('ไม่พบค่า RAW% ที่ถูกต้อง', 'error'); return; }
  const tbody = document.getElementById('batch-tbody'); tbody.innerHTML = '';
  rawValues.forEach((rawPct, i) => {
    const pv = calcPV(rawPct, svh, svl); const bw = Math.min(100, Math.max(0, rawPct));
    const tr = document.createElement('tr');
    tr.innerHTML = `<td style="color:var(--text-dim);font-size:0.8rem">${i+1}</td><td class="td-raw text-right">${rawPct}</td><td class="td-pv text-right">${formatNum(pv)}</td><td class="td-unit">${escHtml(unit)}</td><td><div class="span-bar-wrapper"><div class="span-bar" style="width:${bw}px;max-width:80px"></div><span class="span-pct">${rawPct}%</span></div></td>`;
    tbody.appendChild(tr);
  });
...
document.getElementById('batch-copy-btn').addEventListener('click', () => {
  const rows = document.querySelectorAll('#batch-tbody tr'); if (rows.length === 0) return;
  const unit = document.getElementById('batch-unit').value.trim();
  const lines = ['#\tRAW(%)\tPV\tUnit'];
  rows.forEach((tr, i) => { const tds = tr.querySelectorAll('td'); lines.push(`${i+1}\t${tds[1].textContent.trim()}\t${tds[2].textContent.trim()}\t${unit}`); });
  navigator.clipboard.writeText(lines.join('\n')).then(() => toast('คัดลอกแล้ว', 'success'));
});
...
document.getElementById('batch-export-btn').addEventListener('click', () => {
  ...
  const csvRows = Array.from(rows).map((tr, i) => { const tds = tr.querySelectorAll('td'); return `${i+1},"${tagName}",${tds[1].textContent.trim()},${tds[2].textContent.trim()},"${unit}"`; });
  downloadFile(['#,Tag,RAW(%),PV,Unit', ...csvRows].join('\n'), 'batch_result.csv', 'text/csv');
  toast('Export สำเร็จ', 'success');
});

// UTILITY
function formatNum(n) { if (Number.isInteger(n)) return n.toString(); return parseFloat(n.toFixed(4)).toString(); }
...
function updateTagCount() { document.getElementById('tag-count').textContent = tags.length; }
...
(function init() {
  ...
  if (tags.length === 0) {
    ...
    tags.push(...samples); saveTags(); renderLibrary();
  }
})();
